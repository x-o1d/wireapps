...
function ProductDetails({
  ...
  const addToCart = () => {
    cart.set(p => {
      const itemIndex = p.findIndex(
        i => i.SKU === product?.SKU && i.selectedSize === product?.selectedSize,
      );
      if (itemIndex >= 0) {
        p[itemIndex].count++;
      } else {
  ...
  return (
    <View style={styles.product}>
      <SelectModal ref={selectRef} selectionString="Sizes" />
      <Image style={styles.mainImage} source={{uri: product?.mainImage}} />
      <Text style={styles.nameText}>{product?.name}</Text>
      <Text style={styles.brandText}>by {product?.brandName}</Text>
      <Text style={styles.descriptionText}>{product?.description}</Text>
      <Text style={styles.colourText}>Colour: {product?.colour}</Text>
      <Text style={styles.colourText}>Sizes: {product?.sizes.join(', ')}</Text>
      ...
      <View style={styles.footer}>
        <TouchableOpacity
          ...
          <Text style={styles.sizeButtonText}>
            Size: {product?.sizes[product.selectedSize]}
          </Text>
        </TouchableOpacity>
