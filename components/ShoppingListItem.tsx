...
function ShoppingListItem(props: {
  ...
  useEffect(() => {
    const count = cart.value.reduce((a, c) => {
      if (
        c.SKU === props.item.SKU &&
        c.selectedSize === props.item.selectedSize
      ) {
        return a + c.count;
      } else {
        return a;
  ...
  return (
    <View style={styles.section}>
      <TouchableOpacity
        onPress={() => {
          navigation.navigate('ProductDetails', {SKU: props.item.SKU});
        }}>
      ...
      <View style={styles.buttons}>
        <TouchableOpacity
          onPress={() => {
            props.sizeSelector.current.show(
              props.item.sizes,
              props.item.selectedSize,
              (value: number) => {
                products.set(p => {
                  const productIndex = p.findIndex(
                    i => i.SKU === props.item.SKU,
                  );
